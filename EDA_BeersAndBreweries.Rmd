---
title: "Exploratory Data Analysis of Craft Beers and Breweries"
...
beers = read.csv("Beers.csv", header = TRUE, stringsAsFactors = TRUE)
str(beers)
head(beers)

breweries = read.csv("Breweries.csv", header = TRUE, stringsAsFactors = TRUE)
str(breweries)
head(breweries)
```
...
### Question 3: Here we find the missing values in the dataset and investigate the reason they may be missing and the category of missing values that are in.
```{r question3a, warning = FALSE}
...
#this confirms that if we impute ABV, it will be on 62 obs with imputed IBU
...
### Question 9: Here we look at clusters of states based on their breweries per capita where there where the market might have room for expansion. Then we find the top 10 styles of craft beers in the US overall. We examined if those states were missing popular beer styles and what most produced 2 styles each one of those states might want to introduce. We did utilize generative AI for help with the coding on this one.
```{r question9, message = FALSE}
...
growth_states = breweries_byCap %>% filter(BreweriesPerCap >= 2.0 & BreweriesPerCap <= 3.0)

#filter bbDF for just the growth states
bbDF$State = trimws(bbDF$State)
bbDF_growth_states = bbDF %>%
  filter(State %in% growth_states$State)
...
colnames(growth_states) = c("State", "Population", "Total Breweries", "BreweriesPerMillion", "Percentile")
merge(missing_styles_df, growth_states[,c("State", "BreweriesPerMillion")], by = "State") %>%
  arrange(desc(BreweriesPerMillion)) %>%
  gt() %>%
  tab_header(title = "Missing Styles in Emerging Markets")

#plot states for demo
breweries_byCap$growthState = ifelse(breweries_byCap$State %in% growth_states$State, 1, 0)
breweriesPerCap_plt = breweries_byCap %>% 
  ggplot(aes(x = reorder(State, -BreweriesPerCap), y = BreweriesPerCap, fill = as.factor(growthState))) +
  geom_col() +
  ggtitle("Breweries Per Capita") +
  xlab("State") +
  ylab("Breweries per million people") +
  labs(fill = "Target\nFor Growth") +
  scale_fill_manual(values = c("1" = "red3", "0" = "blue3"),
                    labels = c("1" = "Yes", "0" = "No"),
                    breaks = c("1", "0")) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
...
breweriesPerCap_plt
#ggsave(breweriesPerCap_plt, filename = "plots/breweriesPerCap_plt.png")
```
